...
let installExecuterPluginOnAgent = function (pluginPath, agent) {
  return new Promise((resolve, reject) => {
    let formData = {
      file: {
...
var deleteNode = function (nodeId) {
  return SNode.findOne({
    id: nodeId
  }).populate('children').then((node) => new Promise((res, rej) => {
    if (node.hasChildren) {
      ...
      res(BaseAgent.destroy({
        id: node.data
      }));
    }

  })).then(() => {
    return SNode.destroy({
      id: nodeId
    });
  })
}
  ...
  return new Promise((resolve, reject) => {
    async.each(agentsTree.children, function (child, callback) {
      if (!child.hasChildren) {
        SNode.findOne({
          id: child.id
        }).populate('data').exec(function (err, data) {
          if (err) {
            return callback(err);
          }
          children.push(data);
        ...
        SNode.findOne({
          id: child.id
        }).populate('children').exec(function (err, data) {
          if (err) {
            return callback(err);
          }
          children.push(data);
...
module.exports = {
  ...
  getAgents: function () {
    return SNode.find({
      parent: "-1"
    }).populate('data').populate('children').then((nodes) => new Promise((res, rej) => {
      let trees = [];
  ...
  installExecuterPluginOnAgent: installExecuterPluginOnAgent,
};
