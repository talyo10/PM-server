...
module.exports = {
  ...
  deleteGroup: function (req, res) {
    BaseAgentsService.deleteGroup(req.param('id')).then(() => {
      hooks.deleteServerGroup(req.user, {
        name: req.param('id')
      });
      res.ok();
    }).catch((error) => {
      sails.log.error("Error deleting group", error);
      MessagesService.sendMessage("notification", "Error deleting group", "error");
  ...
  updateAgent: function (req, res) {
    BaseAgentsService.updateBaseAgent(req.body.parentId, req.body.agent).then((agent) => {
      ...
      MessagesService.sendMessage("notification", "Error updating agent", "error");
      res.badRequest();
  ...
  updateSnode: function (req, res) {
    BaseAgentsService.updateSnode(req.body).then((node) => {
      ...
      sails.log.error("Error updating agent", error);
      MessagesService.sendMessage("notification", "Error updating node", "error");
  ...
  getNode: function (req, res) {
    BaseAgentsService.getNode(req.param('id')).then((node) => {
      ...
      MessagesService.sendMessage("notification", "Error getting nodes", "error");
